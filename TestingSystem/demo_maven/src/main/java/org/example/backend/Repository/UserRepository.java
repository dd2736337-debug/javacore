...
import org.example.ultil.JdbcUtils;
...
public class UserRepository implements IUserRepository {
    ...
    public boolean createUser(User user) throws SQLException {
        Connection connection = JdbcUtils.getConnection();
        //khi thêm mới user -> cần truyền vào
        String sql = "insert into Account(email,username,departmentid,passwords,CreateDate,fullname)" +
                "VALUES (?,?,?,?,?,?)";
        PreparedStatement preparedStatement = connection.prepareStatement(sql);
        preparedStatement.setString(1, user.getEmail());
        preparedStatement.setString(2, user.getUserName());
        preparedStatement.setInt(3, user.getDepartment().getDepartmentID());
        preparedStatement.setString(4, user.getPassword());
        preparedStatement.setDate(5, new Date(new java.util.Date().getTime()));
        preparedStatement.setString(6, user.getFullName());

        int ressult = preparedStatement.executeUpdate();
        return ressult > 0;
    }

    @Override
    public boolean changePasswords(String email, String oldPasswords, String newPasswords) throws SQLException {
        Connection connection = JdbcUtils.getConnection();
        String checksql = "SELECT * FROM account WHERE Email = ? AND Passwords = ?";
        PreparedStatement checkpreparedStatement = connection.prepareStatement(checksql);
        checkpreparedStatement.setString(1, email);
        checkpreparedStatement.setString(2, oldPasswords);
        ResultSet resultSet = checkpreparedStatement.executeQuery();
        if (!resultSet.next()) {
            System.out.println("Email hoặc mật khẩu cũ không đúng!");
            return false;
        }
        String updateSQl = "UPDATE account SET Passwords = ? WHERE Email = ?";
        PreparedStatement updatePreparedStatement = connection.prepareStatement(updateSQl);
        updatePreparedStatement.setString(1, newPasswords);
